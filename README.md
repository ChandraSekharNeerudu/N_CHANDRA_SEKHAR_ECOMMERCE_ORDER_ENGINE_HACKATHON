...
Product uses `ReentrantLock` for atomic stock reservation/release.
`ConcurrentHashMap` is used for shared stores.

### Atomic Order Placement

Order placement records rollback actions at each step.
On payment failure, all steps are undone in reverse order.

### State Machine

`OrderStateMachine` enforces valid order lifecycle transitions.
Invalid transitions are blocked.

### Loose Coupling (Microservice Simulation)

`ServiceRegistry` wires all services together.
Each service depends only on interfaces/other services — no direct model access across boundaries.

### Event-Driven

`EventService` maintains an ordered queue.
Events publish on:
...
`ScheduledExecutorService` auto-releases reserved stock after a timeout (30s in demo).

### Idempotency

Unique keys stored in a `Set` — duplicate `placeOrder` calls within a session are rejected.

### Fraud Detection

Tracks per-user order timestamps in a rolling 60-second window.

### Audit Logs

Append-only `ArrayList` — immutable history of all operations.
...
* Reservation expiry timeout = **30 seconds (demo)**
* Payment failure probability ≈ **20%**
* Can be overridden via **Failure Injection (menu option 14)**
* Fraud detection threshold:

  * **3 orders per minute**, OR
  * order value > **₹10,000**
* Partial returns allowed only on:

  * `PAID`
  * `DELIVERED`
* Coupon applied **per cart**, not per item
...
3. Add to Cart → `P001`, qty = 2
   (stock reserved: 2 of 5)
